...
function execute_signal_fn(signal) {
	...
	try {
		...
		if (is_render_effect) {
			res =
				/** @type {(block: import('./types.js').Block) => V} */
				init(
...
function destroy_signal(signal) {
	...
	signal.i =
		...
		signal.y =
		signal.x =
		signal.b = // @ts-expect-error - this is fine, since we're assigning to null to clear out a destroyed signal
		signal.v =
		signal.d =
		signal.c =
			null;
...
function user_effect(init) {
	if (current_effect === null) {
		throw new Error('ERR_SVELTE_ORPHAN_EFFECT');
	}
...
function get_or_init_context_map() {
	...
	if (component_context === null) {
		throw new Error('ERR_SVELTE_ORPHAN_CONTEXT');
	}
...
export {
	...
	subscribe_to_store as Q,
	head as R,
	get_or_init_context_map as S,
	UNINITIALIZED as U,
	set as a,
